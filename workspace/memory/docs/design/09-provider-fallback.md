...
> - `workspace/tools/memory_hook_provider_rollback.py`（46 行）
...
| Provider | 实现来源 | 加载方式 |
|---|---|---|
| `legacy` | gateway 直接 import 的 `build_context_package_core`（gateway.py:33 行定义别名 `CoreBuilder`） | 静态导入，随 gateway 模块加载 |
...
默认 provider 为 `legacy`。环境变量 `MEMORY_HOOK_CORE_PROVIDER` 控制选择（gateway.py:233 行）：
...
`allow_fallback=False` 的唯一已知调用点在 Shadow run 机制中（gateway.py:255 行），用于探测对端 provider 的真实可用性，不允许 fallback 掩盖问题。
...
4. 调用方 `build_context_package()`（gateway.py:234 行）记录这些信息：
...
5. 如果有 provider_errors，且 package 原 status 为 `"ok"`，则将其降级为 `"degraded"`（gateway.py:248-249 行）：
...
文件位于 `workspace/tools/memory_hook_provider_rollback.py`（46 行），是 **一键回滚演练工具**。
...
2. 以 `allow_fallback=True` 分别探测 `"external-core"` 和 `"legacy"` 两个 provider
3. 判定标准：`legacy_probe_provider == "legacy"` → `passed`，否则 `failed`

**判定逻辑的含义**：只要 legacy provider 能正常返回 `"legacy"` 标识，就认为系统具备回退能力。external-core 的探测结果仅作为诊断信息输出，不影响 pass/fail 判定。
...
Shadow run 定义于 `build_context_package()` 函数中（gateway.py:251-270 行）：
...
`MEMORY_HOOK_CORE_PROVIDER` 默认值为 `"legacy"`（gateway.py:233 行），这一设计有明确的稳定性考量：
...
消费者调用 `build_context_package(host, event, payload)` → gateway.py:233 行读取 `MEMORY_HOOK_CORE_PROVIDER`，默认 `"legacy"` → `_resolve_core_builder("legacy", allow_fallback=True)` 直接返回内置 `build_context_package_core` → 构建 context package → 写入 artifact。
