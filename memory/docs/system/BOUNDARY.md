...
| 文档 | `memory/docs/system/` | 仓库级文档（边界、架构、验收报告等） |
...
1. 记录到 `memory/docs/system/RESIDUE_INVENTORY.md`
2. 迁移真实业务数据到对应业务项目仓库
3. 或将内容转为 demo fixture（去除真实业务上下文）
4. 在 `memory/docs/system/RESIDUE_DISPOSITION_PLAN.md` 中跟踪处置进度
...
使用 memory-core `memory-init --sync` 时，项目将生成：
- `.gitlab-ci.yml` 中包含 `sync-to-github` job
- `memory/system/skills/gitlab_sync_workflow.yaml` 作为 submit_gitlab / merge_after_ci / sync_github 的标准编排模板
