...
全局默认策略包位于 `memory-hook-policy-pack.json`（`memory/kb/global/memory-hook-policy-pack.json`）。原 workbot 专用策略包已合并至 memory-hook-policy-pack，scope 字段统一为 `default`。
...
`memory-hook-policy-pack.md`（`memory/kb/global/memory-hook-policy-pack.md`）是 memory 模块级别的策略包规范文档（原 workbot-policy-pack.md 已更新）。
...
`workbot-project-map-governance.md`（`memory/kb/global/workbot-project-map-governance.md`）定义 workbot adapter 级别的项目地图治理规则。
...
`workbot-hook-contract.md`（`memory/kb/global/workbot-hook-contract.md`）定义 Codex 与 Claude 通过 workbot adapter 进入总记忆系统时的 hook 合同。
