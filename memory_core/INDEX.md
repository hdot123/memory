...
- 真相模型 canonical：`memory/kb/global/truth-model.md` — 由各业务项目通过 adapter runtime profile 自行声明（参见仓库根 `README.md` 的 "Adapter Protocol" 与 "Runtime Capabilities" 两节）。memory-core 仓库不内建任何业务项目专属的真相模型文件。
