...
`main()` 函数（[memory_hook_gateway.py:908](<memory-repo>/memory_core/tools/memory_hook_gateway.py:908)）是 CLI 入口。流程如下：
...
`build_context_package()`（[memory_hook_gateway.py:739](<memory-repo>/memory_core/tools/memory_hook_gateway.py:739)）是核心组装函数：
...
`build_context_package_core()`（[memory_hook_core.py:69-271](<memory-repo>/memory_core/tools/memory_hook_core.py:69)）是纯组装逻辑：
...
`write_artifacts()`（[memory_hook_gateway.py:845](<memory-repo>/memory_core/tools/memory_hook_gateway.py:845)）：

1. 调用 `_get_artifact_sink().write(package)`（line 858）
2. 如果 sink 抛出 RuntimeError，走 fallback 路径手动写入（line 860-868）

`ArtifactSinkImpl.write()`（[memory_hook_impls.py:998-1022](<memory-repo>/memory_core/tools/memory_hook_impls.py:998-1022)）：
...
| 常量 | 路径 | 来源 |
|------|------|------|
| `ARTIFACT_ROOT` | `{WORKSPACE_ROOT}/artifacts/memory-hook` | [memory_hook_gateway.py:19](<memory-repo>/memory_core/tools/memory_hook_gateway.py:19) |
| `CONTEXT_ROOT` | `{ARTIFACT_ROOT}/contexts` | [memory_hook_gateway.py:20](<memory-repo>/memory_core/tools/memory_hook_gateway.py:20) |
| `EVENT_LOG` | `{ARTIFACT_ROOT}/events.jsonl` | [memory_hook_gateway.py:21](<memory-repo>/memory_core/tools/memory_hook_gateway.py:21) |

`WORKSPACE_ROOT` = `SCRIPT_PATH.parents[1]`，即 `memory_core/tools/` 的父目录 = `memory_core/`（line 17）。

### 2.2 文件命名规则

Snapshot 文件（[memory_hook_impls.py:1000-1005](<memory-repo>/memory_core/tools/memory_hook_impls.py:1000)）：
...
Latest 文件（[memory_hook_impls.py:1009-1010](<memory-repo>/memory_core/tools/memory_hook_impls.py:1009-1010)）：
- 格式：`latest-{host}-{event}.json`
- 每次写入覆盖，始终指向最近一次该 host+event 组合的 snapshot

### 2.3 Event Log 格式

Event log 是 JSONL 文件（[memory_hook_impls.py:1020-1021](<memory-repo>/memory_core/tools/memory_hook_impls.py:1020)）：
...
`ERROR_LOG` = `{WORKSPACE_ROOT}/memory/system/errors.log`（[memory_hook_gateway.py:22](<memory-repo>/memory_core/tools/memory_hook_gateway.py:22)）

### 3.2 格式

`ErrorSinkImpl.log()`（[memory_hook_impls.py:1038-1040](<memory-repo>/memory_core/tools/memory_hook_impls.py:1038)）：
...
在 `main()` 中（[memory_hook_gateway.py:920-966](<memory-repo>/memory_core/tools/memory_hook_gateway.py:920)）：
...
`build_context_package_core()` 返回的 dict 包含以下顶层 key（[memory_hook_core.py:204-271](<memory-repo>/memory_core/tools/memory_hook_core.py:204)）：
...
- `system_context.core_provider`：实际使用的 provider 名称（[line 791](<memory-repo>/memory_core/tools/memory_hook_gateway.py:791)）
...
- `artifact_refs`：`{"snapshot": str, "latest": str, "event_log": str}`（[memory_hook_impls.py:1010-1014](<memory-repo>/memory_core/tools/memory_hook_impls.py:1010)）

---

## 5 system_context 子结构字段

`system_context` 在 [memory_hook_core.py:222-248](<memory-repo>/memory_core/tools/memory_hook_core.py:222) 构建：
...
`build_context_package()` 额外注入（[memory_hook_gateway.py:790-795](<memory-repo>/memory_core/tools/memory_hook_gateway.py:790)）：
...
`project_context` 在 [memory_hook_core.py:249-259](<memory-repo>/memory_core/tools/memory_hook_core.py:249) 构建：
...
`allowed_reads` 是 `list[str]`（[memory_hook_core.py:154-162](<memory-repo>/memory_core/tools/memory_hook_core.py:154)），包含：
...
`allowed_writes` 是 `dict[str, Any]`，由 `write_targets_fn()` 返回（[memory_hook_core.py:269](<memory-repo>/memory_core/tools/memory_hook_core.py:269)）。

默认实现 `WriteTargetPolicyImpl`（[memory_hook_impls.py:392-417](<memory-repo>/memory_core/tools/memory_hook_impls.py:392)）返回：
...
`validation_errors` 是扁平的 `list[str]`，在 [memory_hook_core.py:215-221](<memory-repo>/memory_core/tools/memory_hook_core.py:215) 汇总：
...
在 [memory_hook_gateway.py:798-803](<memory-repo>/memory_core/tools/memory_hook_gateway.py:798) 中：
...
```
（[memory_hook_core.py:185-194](<memory-repo>/memory_core/tools/memory_hook_core.py:185)）
...
hook-state.json 的顶层结构（[cmux_hook_state.py:74-79](<memory-repo>/memory_core/tools/cmux_hook_state.py:74)）：
...
每个 surface 的状态字段（[cmux_hook_state.py:176-189](<memory-repo>/memory_core/tools/cmux_hook_state.py:176)）：
...
`record_hook_event()`（[cmux_hook_state.py:160-225](<memory-repo>/memory_core/tools/cmux_hook_state.py:160)）：
...
`_write_hook_state_unlocked()`（[cmux_hook_state.py:121-140](<memory-repo>/memory_core/tools/cmux_hook_state.py:121)）：
...
`ClaudeDelegate` 在 `execute()` 中调用 `record_hook_event()`（[memory_hook_impls.py:149-159](<memory-repo>/memory_core/tools/memory_hook_impls.py:149)）：
