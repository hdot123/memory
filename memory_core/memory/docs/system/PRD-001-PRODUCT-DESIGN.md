...
| 类别 | 命令 | 用途 |
|------|------|------|
| **核心** | `memory-init` | 初始化（create/adopt/update/repair 四模式） |
| | `memory-validate` | 验证完整性 |
| | `memory-migrate` | 版本迁移 |
| **Hook** | `memory-hook-gateway` | Hook 网关入口 |
| | `memory-codex-hooks` | Codex 全局 hook |
| | `memory-claude-hooks` | Claude 全局 hook |
| | `memory-factory-hooks` | Factory 全局 hook |
| **保护** | `memory-integrity-resign` | 重新签名 |
| | `memory-ownership` | 所有权管理（show/validate/plan-update/apply-update） |
| **审计** | `memory-audit-layout` | 布局审计 |
| | `memory-plan-residue` | 残留处理计划 |
| | `memory-apply-residue-plan` | 应用残留处理计划 |
| | `memory-consistency-check` | 跨平台一致性检查 |
| | `memory-verify-consumer` | 消费者验证 |
...
| **v0.6.0** | 能力注册表 | CAPABILITY_REGISTRY + 上下文生命周期管理（TTL/提炼/压缩）+ Hook 协议版本化 |
