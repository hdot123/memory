...
def _run_layout_audit(target: Path) -> dict[str, Any] | None:
    ...
    try:
        ...
        recommended_mode = _determine_recommended_mode(
            ...
            has_dot_memory=(target / ".memory").exists(),
        )
...
def _has_workspace_memory_conflict(target: Path, findings: list) -> bool:
    ...
    Real conflict = root memory structures (.memory/, memory/, project-map/)
    coexist with workspace structures (workspace/memory/, workspace/project-map/).

    Having just .memory/ + memory/ + project-map/ without workspace
    ...
    has_root_dot_memory = (target / ".memory").exists()
    has_root_memory = (target / "memory").exists()
    has_root_project_map = (target / "project-map").exists()

    root_structures = has_root_dot_memory or has_root_memory or has_root_project_map
...
def _determine_recommended_mode(
    ...
    Modes:
        ...
        - adopt: Has .memory structure but some legacy residue
        - update: Clean .memory structure, no conflicts
    ...
    if total == 0:
        ...
        return "fresh" if not has_dot_memory else "update"

    if p0 > 0:
        # Critical issues (but not workspace conflicts)
        if not has_dot_memory:
            return "adopt"
        return "repair"
    ...
    if p2 > 0 or root_pollution_count > 0:
        ...
        return "repair" if has_dot_memory else "adopt"
