...
def get_default_adapter_required_canonical() -> list[str]:
    ...
    # Look for patterns like: project_root / "memory" / "system" / "CANONICAL.md"
    if 'project_root / "memory" / "system" / "CANONICAL.md"' in content:
        required.append("memory/system/CANONICAL.md")
    if 'project_root / "memory" / "system" / "PLAN.md"' in content:
        required.append("memory/system/PLAN.md")
    if 'project_root / "memory" / "system" / "STATE.md"' in content:
        required.append("memory/system/STATE.md")
...
@pytest.mark.skip(reason="CANONICAL.md, PLAN.md, STATE.md removed in v0.5.0")
class TestInitGeneratesRequiredCanonical:
    ...
    def test_init_generates_all_required_canonical(self, tmp_path: Path) -> None:
        ...
        - memory/system/CANONICAL.md
        - memory/system/PLAN.md
        - memory/system/STATE.md
        ...
        required_canonical = [
            "CANONICAL.md",  # in memory/system/
            "PLAN.md",       # in memory/system/
            "STATE.md",      # in memory/system/
        ]

        # Check memory/system/ files
        for req_file in required_canonical:
            full_path = f"memory/system/{req_file}"
