...
def get_default_adapter_required_canonical() -> list[str]:
    ...
    # which map to memory/system/kb/global/ paths (system-level)
    if 'truth_model' in content:
        required.append("memory/system/kb/global/truth-model.md")
    if 'memory_system_path' in content:
        required.append("memory/system/kb/global/memory-system.md")
    if 'global_rule_path' in content:
        required.append("memory/system/kb/global/memory-routing.md")
...
class TestInitGeneratesRequiredCanonical:
    ...
    def test_init_generates_all_required_canonical(self, tmp_path: Path) -> None:
        ...
        # Check memory/system/kb/global/ files (system-level governance stubs)
        # These are now created at system level instead of memory/kb/global/
        # Note: dry-run doesn't create directories on disk, check the output instead
        would_create_dirs = dry_run_output.get("would_create_dirs", [])
        has_system_global_dir = any(
            "memory/system/kb/global" in d or d == "memory/system/kb/global"
...
class TestInitGeneratesPolicyPack:
    """Test that init creates memory/system/kb/global/ directory (system-level)."""

    def test_init_generates_policy_pack(self, tmp_path: Path) -> None:
        """Test that init creates memory/system/kb/global/ directory.

        VAL-INIT-001: The directory structure must NOT include memory/kb/global/
        but must include memory/system/kb/global/ (system-level governance).
        """
        result = run_init_dry_run(tmp_path, scope="test-project")
        dry_run_output = result.get("dry_run_output", {})
        would_create_dirs = dry_run_output.get("would_create_dirs", [])

        # System-level global directory should be created
        has_system_global_dir = any(
            "memory/system/kb/global" in d or d == "memory/system/kb/global"
            for d in would_create_dirs
        )

        assert has_system_global_dir, \
            "memory/system/kb/global directory should be created (system-level governance)"

        # Project-level memory/kb/global should NOT be created (VAL-INIT-001)
        has_project_global_dir = any(
            d == "memory/kb/global"
            for d in would_create_dirs
        )
        assert not has_project_global_dir, \
            "memory/kb/global directory should NOT be created (project-level global removed)"
...
class TestInitRuntimeCoverageComprehensive:
    ...
    def test_init_creates_all_essential_directories(self, tmp_path: Path) -> None:
        ...
        # VAL-INIT-001: memory/kb/global should NOT be created
        assert "memory/kb/global" not in would_create_dirs, \
            "memory/kb/global should NOT be created (project-level global removed)"
    ...
    def test_no_degraded_due_to_missing_init_files(self, tmp_path: Path) -> None:
        ...
        # VAL-INIT-001: memory/kb/global/ should NOT exist
        assert not (tmp_path / "memory" / "kb" / "global").exists(), \
            "memory/kb/global/ should NOT be created by init"

        # memory/system/kb/global/ SHOULD exist (system-level governance)
        assert (tmp_path / "memory" / "system" / "kb" / "global").exists(), \
            "memory/system/kb/global/ should be created by init"
