...
def get_default_adapter_required_canonical() -> list[str]:
    ...
    # Look for patterns like: project_root / ".memory" / "CANONICAL.md"
    if 'project_root / ".memory" / "CANONICAL.md"' in content:
        required.append(".memory/CANONICAL.md")
    if 'project_root / ".memory" / "PLAN.md"' in content:
        required.append(".memory/PLAN.md")
    if 'project_root / ".memory" / "STATE.md"' in content:
        required.append(".memory/STATE.md")
...
# ---------------------------------------------------------------------------
# Test 5: Init generates NOW.md
# ---------------------------------------------------------------------------

class TestInitGeneratesNowMd:
    """Test that init generates the NOW.md file."""

    def test_init_generates_now_md(self, tmp_path: Path) -> None:
        """Test that init generates workspace root NOW.md.

        The runtime reads workspace_root / "NOW.md" as state_entry.
        This is a critical file for the memory system.
        """
        result = run_init_dry_run(tmp_path, scope="test-project")
        dry_run_output = result.get("dry_run_output", {})
        would_create_files = dry_run_output.get("would_create_files", [])
        ...
        # NOW.md should be in the workspace root
        assert "NOW.md" in created_files, \
            "NOW.md should be generated by init in workspace root"
...
class TestInitRuntimeCoverageComprehensive:
    ...
    def test_no_degraded_due_to_missing_init_files(self, tmp_path: Path) -> None:
        ...
        required_files = [
            tmp_path / ".memory" / "CANONICAL.md",
            tmp_path / ".memory" / "PLAN.md",
            tmp_path / ".memory" / "STATE.md",
            tmp_path / ".memory" / "TASKS.md",
            tmp_path / ".memory" / "memory.lock",
            tmp_path / ".memory" / "adapter.toml",
