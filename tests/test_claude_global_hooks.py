...
from memory_core.tools.claude_global_hooks import install_claude_hooks, merge_claude_hooks
...
def test_merge_replaces_existing_memory_hooks_and_preserves_unrelated(tmp_path: Path) -> None:
    """Test that merge replaces old memory hooks while preserving unrelated hooks."""
    wrapper = tmp_path / ".claude" / "bin" / "memory-hook"
    existing = {
        "hooks": [
            {"event": "SessionStart", "command": "custom-session-start", "stdin": True},
            {
                "event": "SessionStart",
                "command": "python3 /Users/busiji/workbot/workspace/tools/memory_hook_gateway.py --host claude --event session-start",
                "stdin": True,
            },
            {"event": "CustomEvent", "command": "keep-me", "stdin": False},
        ]
    }

    merged = merge_claude_hooks(
        existing,
        {
            "hooks": [
                {"event": "SessionStart", "command": f"{wrapper} --host claude --event session-start", "stdin": True},
                {"event": "UserPromptSubmit", "command": f"{wrapper} --host claude --event prompt-submit", "stdin": True},
                {"event": "Notification", "command": f"{wrapper} --host claude --event notification", "stdin": True},
                {"event": "Stop", "command": f"{wrapper} --host claude --event stop", "stdin": True},
            ]
        },
    )

    # Check that old bare gateway commands are filtered out
    commands = [h["command"] for h in merged["hooks"]]
    assert "custom-session-start" in commands
    assert "keep-me" in commands
    assert all("workbot/workspace/tools/memory_hook_gateway.py" not in cmd for cmd in commands)
    assert f"{wrapper} --host claude --event session-start" in commands
    assert f"{wrapper} --host claude --event prompt-submit" in commands


def _fake_memory_commands(tmp_path: Path, monkeypatch) -> tuple[Path, Path]:
    """Create fake gateway and init commands for testing."""
...
def test_install_claude_hooks_writes_wrapper_and_hooks_json(monkeypatch, tmp_path: Path) -> None:
    """Test that install creates wrapper and hooks.json with correct content."""
    claude_home = tmp_path / ".claude"
    storage_root = tmp_path / "memory-store"
    gateway, init = _fake_memory_commands(tmp_path, monkeypatch)

    result = install_claude_hooks(claude_home=claude_home, storage_root=storage_root)

    wrapper = claude_home / "bin" / "memory-hook"
    hooks_path = claude_home / "hooks.json"
    ...
    commands = [h["command"] for h in hooks["hooks"]]
    assert len(commands) == 4  # SessionStart, UserPromptSubmit, Notification, Stop
    assert all(str(wrapper) in command for command in commands)
    assert all("--host claude --event" in command for command in commands)


def test_wrapper_skips_exact_home_project_root_but_allows_child(monkeypatch, tmp_path: Path) -> None:
    """Test that wrapper skips HOME but allows child projects."""
    claude_home = tmp_path / ".claude"
    ...
    install_claude_hooks(claude_home=claude_home, storage_root=tmp_path / "global-state")
    wrapper = claude_home / "bin" / "memory-hook"

    home_proc = subprocess.run(
        [str(wrapper), "--host", "claude", "--event", "session-start"],
        cwd=fake_home,
        text=True,
        capture_output=True,
        check=False,
...
def test_wrapper_initializes_project_memory_before_gateway(monkeypatch, tmp_path: Path) -> None:
    """Test that wrapper initializes project memory before calling gateway."""
    claude_home = tmp_path / ".claude"
...
def test_wrapper_initializes_git_project_root_from_subdirectory(monkeypatch, tmp_path: Path) -> None:
    """Test that wrapper normalizes to git root from subdirectory."""
    claude_home = tmp_path / ".claude"
    project = tmp_path / "project"
...
def test_wrapper_skips_auto_init_for_memory_core_source_repo(monkeypatch, tmp_path: Path) -> None:
    """Test that wrapper skips memory-core source repo to prevent self-pollution."""
    claude_home = tmp_path / ".claude"
...
def test_wrapper_skips_memory_core_source_repo_even_with_dot_memory(monkeypatch, tmp_path: Path) -> None:
    ...
    nested.mkdir(parents=True)
    (nested / "memory_hook_gateway.py").write_text("# marker\n", encoding="utf-8")
    (nested / "claude_global_hooks.py").write_text("# marker\n", encoding="utf-8")
    subprocess.run(["git", "init"], cwd=memory_repo, check=True, capture_output=True, text=True)
...
def test_wrapper_detects_memory_core_by_claude_global_hooks(monkeypatch, tmp_path: Path) -> None:
    """Anti-pollution: wrapper should detect memory-core by claude_global_hooks.py."""
    claude_home = tmp_path / ".claude"
    memory_repo = tmp_path / "memory-core"
    nested = memory_repo / "memory_core" / "tools"
    nested.mkdir(parents=True)
    # Only claude_global_hooks.py marker
    (nested / "claude_global_hooks.py").write_text("# marker\n", encoding="utf-8")
    subprocess.run(["git", "init"], cwd=memory_repo, check=True, capture_output=True, text=True)
    _fake_memory_commands(tmp_path, monkeypatch)
...
def test_install_claude_hooks_fails_without_installed_gateway(monkeypatch, tmp_path: Path) -> None:
    """Test that install fails gracefully when gateway is not found."""
    monkeypatch.setenv("PATH", str(tmp_path / "empty-bin"))

    result = install_claude_hooks(claude_home=tmp_path / ".claude", dry_run=True)
...
def test_install_claude_hooks_backs_up_existing_hooks_json(monkeypatch, tmp_path: Path) -> None:
    """Test that install backs up existing hooks.json before modifying."""
    claude_home = tmp_path / ".claude"
    claude_home.mkdir()
    hooks_path = claude_home / "hooks.json"
    existing_hooks = '{"hooks":[{"event":"SessionStart","command":"keep-me","stdin":true}]}\n'
    hooks_path.write_text(existing_hooks, encoding="utf-8")
    _fake_memory_commands(tmp_path, monkeypatch)

    result = install_claude_hooks(claude_home=claude_home, storage_root=tmp_path / "store")
...
def test_project_lifecycle_reuses_git_identity_after_project_path_is_deleted(tmp_path: Path) -> None:
    """Test that lifecycle records are preserved even when project is deleted."""
    ...
    assert saved["first_observed_at"] == "2026-05-12T00:00:00+08:00"


def test_project_lifecycle_missing_path_preserves_existing_record(tmp_path: Path) -> None:
    """Test that missing path preserves existing lifecycle record."""
    lifecycle_root = tmp_path / "lifecycle"
